...
namespace DuiLib
{
	class LuaControlDelegate
	{
		int luaFunc = LUA_NOREF;
		CControlUI* pControl = nullptr;
	public:
		LuaControlDelegate(CControlUI* _pControl, int pos):pControl(_pControl)
		{
			assert(globalLuaEnv);
			if (globalLuaEnv)
			{
				lua_pushvalue(*globalLuaEnv, pos);
				luaFunc = luaL_ref(*globalLuaEnv, LUA_REGISTRYINDEX);
			}
		}
		~LuaControlDelegate()
		{
			pControl = nullptr;
			if (globalLuaEnv && luaFunc != LUA_NOREF)
			{
	...
	public:
		...
		{
			return OnAction<TEventUI>(event);
		}
		bool OnHandleNotify(void* msg)
		{
			return OnAction<TNotifyUI>(msg);
		}
		bool OnHandleControl(void* param)
		{
			return OnAction<CControlUI>(param);
		}
	private:
		template<typename T>
		inline bool OnAction(void* param)
		{
	...
	class AllLuaControlDelegate
	{
		std::set<LuaControlDelegate*> m_all_delegates;
	public:
		AllLuaControlDelegate() {}
		~AllLuaControlDelegate()
		{
			for (auto it = m_all_delegates.begin(); it != m_all_delegates.end(); ++it)
			{
				LuaControlDelegate* d = *it;
				delete d;
			}
			m_all_delegates.clear();
		}

		void add_delegate(LuaControlDelegate* d)
		{
			if (m_all_delegates.find(d) == m_all_delegates.end())
				m_all_delegates.insert(d);
		}
		void del_delegate(LuaControlDelegate* d)
		{
			auto it = m_all_delegates.find(d);
			if (it != m_all_delegates.end())
			{
				m_all_delegates.erase(it);
				delete d;
			}
		}
	};
...
namespace DuiLib
{
	...
	static int CControlUI_OnInitRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
		assert(pDelegate);
		if (!pDelegate) return 0;
		pControl->OnInit -= MakeDelegate(pDelegate, &LuaControlDelegate::OnHandleControl);
		all_delegates.del_delegate(pDelegate);
		return 0;
	}
	static int CControlUI_OnDestroyAdd(lua_State* l)
	{
		...
		LuaControlDelegate* pDelegate = new LuaControlDelegate(pControl, 2);
		all_delegates.add_delegate(pDelegate);
		pControl->OnDestroy += MakeDelegate(pDelegate, &LuaControlDelegate::OnHandleControl);
		lua::push(l, pDelegate);
		return 1;
	}
	static int CControlUI_OnDestroyRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
	...
	static int CControlUI_OnSizeRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
	...
	static int CControlUI_OnEventRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
	...
	static int CControlUI_OnNotifyRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
	...
	static int CControlUI_OnPaintRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
	...
	static int CControlUI_OnPostPaintRemove(lua_State* l)
	{
		CControlUI* pControl = nullptr;
		LuaControlDelegate* pDelegate = nullptr;
		lua::get(l, 1, &pControl, &pDelegate);
...
#define WRAP_METHOD(m) .def(#m, &CControlUI::m)
		lua::lua_register_t<CControlUI>(l, "DuiLib.CControlUI", DeleteControl<CControlUI>)
			...
			LAMBDA_METHOD2("OnInitRemove", CControlUI_OnInitRemove)
			LAMBDA_METHOD2("OnDestroyAdd", CControlUI_OnDestroyAdd)
			LAMBDA_METHOD2("OnDestroyRemove", CControlUI_OnDestroyRemove)
			LAMBDA_METHOD2("OnSizeAdd", CControlUI_OnSizeAdd)
			LAMBDA_METHOD2("OnSizeRemove", CControlUI_OnSizeRemove)
			LAMBDA_METHOD2("OnEventAdd", CControlUI_OnEventAdd)
			LAMBDA_METHOD2("OnEventRemove", CControlUI_OnEventRemove)
			LAMBDA_METHOD2("OnNotifyAdd", CControlUI_OnNotifyAdd)
			LAMBDA_METHOD2("OnNotifyRemove", CControlUI_OnNotifyRemove)
			LAMBDA_METHOD2("OnPaintAdd", CControlUI_OnPaintAdd)
			LAMBDA_METHOD2("OnPaintRemove", CControlUI_OnPaintRemove)
			LAMBDA_METHOD2("OnPostPaintAdd", CControlUI_OnPostPaintAdd)
			LAMBDA_METHOD2("OnPostPaintRemove", CControlUI_OnPostPaintRemove)
...
namespace DuiLib
{
	static int RegisterWindowMessageByLua(lua_State* l)
	{
		CDuiString message;
		lua::get(l, 1, &message);
		return lua::push(l, ::RegisterWindowMessage(message));
	}
	static int LoadIconFromFile(lua_State* l)
	{
		HINSTANCE hInstance = nullptr;
		CDuiString pstrFileName;
	...
	static int DestroyIconByLua(lua_State* l)
	{
		HICON hIcon = nullptr;
		lua::get(l, 1, &hIcon);
		::DestroyIcon(hIcon);
	...
	static int GetCursorPointByLua(lua_State* l)
	{
		POINT pt;
		::GetCursorPos(&pt);
	...
	static int GetModulePathByLua(lua_State* l)
	{
	...
	static int GetAPPDATAPathByLua(lua_State* l)
	{
	...
	static int CreateDirectoryByLua(lua_State* l)
	{
		CDuiString strPath;
		lua::get(l, 1, &strPath);
		bool ret = true;
	...
	static int IsFileExistByLua(lua_State* l)
	{
		CDuiString strPath;
		lua::get(l, 1, &strPath);
	...
	static int IsDirectoryExistByLua(lua_State* l)
	{
		CDuiString strPath;
		lua::get(l, 1, &strPath);
	...
	static void FindImportFiles(CDuiString strPath, std::vector<CDuiString>& importfiles)
	{
	...
	static int SHBrowseForFolderByLua(lua_State* l)
	{
		HWND hWnd = NULL;
		CDuiString strTitle;
		...
		if (pIDList)
		{
			...
			{
				FindImportFiles(Buffer, filelist);
			}
		}
	...
	static int RegSetByLua(lua_State* l)
	{
		HKEY hKey = NULL;
	...
	static int RegSetExByLua(lua_State* l)
	{
		HKEY hKey = NULL;
		CDuiString cs_subkey;
		CDuiString cs_value_name;
	...
	static int RegDeleteByLua(lua_State* l)
	{
	...
	static int RegQueryByLua(lua_State* l)
	{
		CDuiString cs_subkey, cs_value_name;
		lua::get(l, 1, &cs_subkey, &cs_value_name);
		HKEY hKEY;
	...
	static int GetAppNameByLua(lua_State* l)
	{
	...
	static int RegisterHotKeyByLua(lua_State* l)
	{
		DWORD dwkey;
		HWND hWnd = nullptr;
		...
		UINT fsModifiers = HKF2SDK(wModifiers);
		bool ret = ::RegisterHotKey(hWnd, dwkey, fsModifiers, wVirtualKeyCode);
		return lua::push(l, ret);
	}
	static int UnregisterHotKeyByLua(lua_State* l)
	{
		DWORD dwkey;
		HWND hWnd = nullptr;
	...
	static int FindFirstFileByLua(lua_State* l)
	{
		CDuiString strPath;
		lua::get(l, 1, &strPath);
	...
	static int FindNextFileByLua(lua_State* l)
	{
	...
	static int GetFindDataByLua(lua_State* l)
	{
		WIN32_FIND_DATA* FIND_DATA = NULL;
		lua_pushnil(l);
		int real_pos = 1;
	...
	static int FindCloseByLua(lua_State* l)
	{
		WIN32_FIND_DATA* FIND_DATA = NULL;
		HANDLE h_find = INVALID_HANDLE_VALUE;
	...
	static void RegisterWin32APIToLua(lua_State* l)
	{
		//win32
		lua::lua_register_t<void>(l, "win32")
			.def("RegisterWindowMessage", RegisterWindowMessageByLua)
			.def("LoadIconFromFile", LoadIconFromFile)
			.def("DestroyIcon", DestroyIconByLua)
			.def("GetCursorPos", GetCursorPointByLua)
			.def("GetModulePath", GetModulePathByLua)
			.def("GetAPPDATAPath", GetAPPDATAPathByLua)
			.def("CreateDirectory", CreateDirectoryByLua)
			.def("SHBrowseForFolder", SHBrowseForFolderByLua)
			.def("IsDirectoryExist", IsDirectoryExistByLua)
			.def("IsFileExist", IsFileExistByLua)
			.def("RegSet", RegSetByLua)
			.def("RegSetEx", RegSetExByLua)
			.def("RegDelete", RegDeleteByLua)
			.def("RegQuery", RegQueryByLua)
			.def("GetAppName", GetAppNameByLua)
			.def("RegisterHotKey", RegisterHotKeyByLua)
			.def("UnregisterHotKey", UnregisterHotKeyByLua)
			.def("FindFirstFile", FindFirstFileByLua)
			.def("FindNextFile", FindNextFileByLua)
			.def("GetFindData", GetFindDataByLua)
			.def("FindClose", FindCloseByLua);
