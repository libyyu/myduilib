		...
		case EM_LIST_ORDER://顺序播放
			{
				...
				{
					size_t nIndex = GetCurIndexByCurSong(songList);
					nIndex ++;
					if (nIndex<nSize)
					{
...
{
	ASSERT(path);
	DuiLib::CDuiString szPath(path);
	spSongInfoT spSong;
	CSongInfo song;
	AudioTagInfo * ptagInfo = new AudioTagInfo;
#define MB_2_UTF8(x) DuiLib::Convert::MBToUTF8(TransToStdString(x).c_str(), -1)
	ptagInfo->init(MB_2_UTF8(szPath));

	song.SetSongName(TransToDuiString(ptagInfo->title().c_str()));
	if (song.GetSongName().IsEmpty())
	{
		song.SetSongName(em_utility::mci::rip_file_name(path,false).c_str());
	}
	song.SetAblumName(TransToDuiString(ptagInfo->album().c_str()));
	song.SetArtistName(TransToDuiString(ptagInfo->artist().c_str()));
	song.SetLocalPath(path);
	song.SetLocal(true);
	song.SetAddTime(em_utility::mci::now_time());
	song.SetFileSize(em_utility::mci::get_file_size(path));
	song.SetFileStartPos(0);
	song.SetFileExt(em_utility::mci::rip_file_ext(path,true).c_str());
	song.SetRealArtistName(TransToDuiString(ptagInfo->artist().c_str()));
	song.SetEnable(true);
...
void PlayListMgr::CheckSong(spSongInfoT& spSong)
{
	...
	spSong->SetSongName(spSong->GetSongName().IsEmpty()? em_utility::mci::rip_file_name(spSong->GetLocalPath(),false).c_str() : spSong->GetSongName());
