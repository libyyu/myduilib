...
bool HTTPprotocol::Initialize(const char * AszUrlBase) {
...
void HTTPprotocol::SpaceTransformation(HTTPprotocol::String& str )
{
...
bool HTTPprotocol::CreateCurl( void )
{
...
void HTTPprotocol::ReleaseCurl( void )
{
	if (m_pCurl)
	{
		// 清空;
...
//成功返回0，失败返回1
int HTTPprotocol::DownloadFile(const char * AszFileName, const char * AszTempFilePath,ProgressCallback progessProc,void* usrdata) {
...
bool HTTPprotocol::sslpost( const HTTPprotocol::String& ulr,const char* _Content )
{
	if (!_Content)
	{
		return false;
...
long HTTPprotocol::getDownloadFileLenth(const HTTPprotocol::String &url)
{
...
long HTTPprotocol::getLocalFileLenth(const HTTPprotocol::String &localfile)
{
...
long HTTPprotocol::getDownloadFileLenth()
{
	return m_totalSize;
}

void HTTPprotocol::Pause()
{
	if(m_pCurl)
	{
