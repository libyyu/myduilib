...
#include "Utility/win_io/win_io_base.h"
#include "Marco.hpp"
#include "YMusic.h"
#include "YApp.h"
#include <wininet.h>
...
bool down_http_file::find_task(const down_task& task)
{
   ...
   {
		down_task* p_cur = &*i;
		if (p_cur->s_url==task.s_url && p_cur->task_type==task.task_type)		//�Ѿ����������������
		{
			b_find = true;
			break;
		}
...
{	
	...
	int n_path=task.s_local_file_down.rfind(_T('\\'));
	if (n_path < 0)			//����ЧĿ¼
	{
		n_path=task.s_local_file_down.rfind(_T('/'));
	}
	if (n_path < 0)
	{
		return;
	}
	stringtype cs_path = task.s_local_file_down.substr(0, n_path);		//task.s_local_file.Left(n_path+1);//�޸���2009-6-11
	static const int DRIVER_ROOT_LENGTH = 2;			//sample "d:"
	if (cs_path.length() == DRIVER_ROOT_LENGTH)
		cs_path += _T("\\");
	if (!mci::is_directory(cs_path.c_str()))
	{
		mci::confirm_dir(cs_path.c_str(),_T('/'));
	}
...
down_http_file::down_result_e down_http_file::down_file_sync(const down_http_file::stringtype& s_url, const down_http_file::stringtype& s_local_file)
{
	DWORD dwFlags = 0;
	...
	if (h_open != NULL)					//��internet
	{
		...
		HINTERNET h_connect = ::InternetOpenUrl(h_open, (LPCTSTR) s_url.c_str(),  COMMON_HEADER,
		...
		int n_result = 0;
		n_result = local_file.create(s_local_file.c_str());
		if (n_result != 0)
		{
			local_file.close();
			::InternetCloseHandle(h_connect);
...
down_http_file::down_result_e down_http_file::http_file_down(const down_http_file::stringtype& s_url, const down_http_file::stringtype& s_local_file)
{
	DWORD dwFlags = 0;
	::InternetGetConnectedState(&dwFlags, 0);
	if (dwFlags & INTERNET_CONNECTION_OFFLINE)		//take appropriate steps
...
//  int e_result = HttpManagerProto::Instance()->DoHttpGet(pdata);
 	...
 	if(e_result == dr_success)
	{
		...
		if(!p_task->s_local_file_normal.empty())
		{
			BOOL bResult = ::MoveFile(p_task->s_local_file_down.c_str(),p_task->s_local_file_normal.c_str());
			bResult ? sLog(_T("::MoveFile[%s ---> %s]"),
				p_task->s_local_file_down.c_str(),
				p_task->s_local_file_normal.c_str()) : sLogError(_T("::MoveFile[%s--->%s]"),
				p_task->s_local_file_down.c_str(),
				p_task->s_local_file_normal.c_str());
		}
