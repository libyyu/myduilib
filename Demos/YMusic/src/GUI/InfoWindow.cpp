...
void CInfoWindow::ShowInfoWindow(RECT rt,LPCTSTR pszText)
{
	CInfoWindow::ShowInfoWindow(theMainWin->GetHWND(),rt,pszText);
}

void CInfoWindow::ShowInfoWindow(POINT pt, LPCTSTR pszText)
{
	CInfoWindow::ShowInfoWindow(theMainWin->GetHWND(), pt, pszText);
}
...
void CInfoWindow::SetText(LPCTSTR pszText)
{
	...
	HDC dc = ::GetDC(m_hWnd);
	RECT rt = {0};
	::DrawText(dc,pszText,_tcslen(pszText),&rt,DT_CALCRECT);
	::ReleaseDC(m_hWnd,dc);
...
{
	this->RegisterSkin(_T("xml/InfoWnd.xml"),0, this);
	return 1;
}
