...
void CLrcToolBar::Notify(DuiLib::TNotifyUI& msg)
{
	...
	else if (0 == msg.sType.Compare(DUI_MSGTYPE_CLICK))
	{
		...
		else if (pSender == _ui._pLockBtn)
		{
			...
			ShowWindow(false);
			theMainWin->ShowTrayTooltip(_T("������������,������ͨ���Ҽ�������"),_T("����"));
			sLog(_T("CLrcToolBar::Notify %s,lock lrc"),pSender->GetName().GetData());
		}
		else if (pSender== _ui._pPanelLrcBtn)
		{
			...
			sLog(_T("CLrcToolBar::Notify %s,ShowPanelLrcTab"),pSender->GetName().GetData());
		}
		else if (pSender == _ui._pCloseSysBtn)
		{
			if(m_pDeskLrc)::SendMessage(m_pDeskLrc->GetHWND(),WM_SYSCOMMAND,SC_MINIMIZE,0);
			sLog(_T("CLrcToolBar::Notify %s,HideDeskLrc"),pSender->GetName().GetData());
		}	
	}
	else if (0 == msg.sType.Compare(DUI_MSGTYPE_SELECTCHANGED))
	{
		...
		{
			bool bChecked = static_cast<DuiLib::CFadeCheckBoxUI*>(pSender)->IsSelected();
			theConfigMgr->GetNormalConfig()->SetKaraokMode(bChecked);
			sLog(_T("CLrcToolBar::Notify %s,SetKaraokMode(%d)"),pSender->GetName().GetData(),bChecked);
		}
	}
}
...
{
	sLog(_T("CWebCLrcToolBarLoginWindow::OnCreate"));

	ModifyStyle(WS_CAPTION,WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
	if(!RegisterSkin(_T("xml/lrc-toolbar.xml"), 0, this))
		return 0;
	DuiLib::CDuiRect rcBar;
	GetWindowRect(&rcBar);
	BOOL nRet = ::SetWindowPos(*this, HWND_TOPMOST, rcBar.left, rcBar.top, rcBar.GetWidth(), rcBar.GetHeight(),SWP_NOACTIVATE|SWP_NOSENDCHANGING );
...
BOOL CLrcToolBar::Receive(SkinChangedParam param)
{
	...
	{
		sLogError(_T("CLrcToolBar::Receive"));
