...
CSysSkinLayoutUI::CSysSkinLayoutUI()
{
	...
	builder.Create(_T("xml/SysSkinLayout.xml"),0,NULL,NULL,this);
	_ImageCount = 0;
	Init();
}
CSysSkinLayoutUI::~CSysSkinLayoutUI()
...
void CSysSkinLayoutUI::AddImage(const DuiLib::CDuiString& szImage)
{
	ASSERT(szImage);
	DuiLib::CDuiString sImage;
	sImage.Format(_T("file='%s' dest='8,8,132,92' corner='5,5,5,5'"),szImage.GetData());
	DuiLib::CDuiString sUserData;
	sUserData.Format(_T("file='%s' corner='5,5,5,5'"),szImage.GetData());
