	...
	:_pfile(NULL),_level(lev),_type(type)
{
	...
	_tcscpy_s(_filename,512,filename);
	_tcscpy_s(_dirname,512,dirname);
...
bool CLog::_Open()
{
	...
	MYSTDSTRING stmp(_timename);
	stmp += _filename;
	MYSTDSTRING sfile(_dirname);

	sfile += stmp;
	int er;
	errno_t err = _tfopen_s(&_pfile,sfile.c_str(),_T("a"));
	if (_pfile == 0)
	{
		er = GetLastError();
	}
...
void CLog::_NewLog(tm* atm)
{
	...
	{
		_tcscpy_s(_timename,512,buffer);
		_Close();
		_Open();
	}
}
