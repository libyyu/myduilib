...
static bool OnLogoButtonEvent(void* event) {
	...
	else if (((TEventUI*)event)->Type == UIEVENT_DBLCLICK) {
		CControlUI* pButton = ((TEventUI*)event)->pSender;
		if (pButton != NULL) {
			CListContainerElementUI* pListElement = (CListContainerElementUI*)(pButton->GetTag());
			if (pListElement != NULL) pListElement->DoEvent(*(TEventUI*)event);
...
User CFriendsUI::GetUser(Node* node)
{
	...
	CDuiString sIpAndPort = node->data().value;
	int iPos = sIpAndPort.Find(_T("_"));
	if(iPos>-1)
	{
		CDuiString sIp = sIpAndPort.Left(iPos-1);
		CDuiString sPort = sIpAndPort.Right(sIpAndPort.GetLength()-iPos);
		user.m_userIp = sIp;
