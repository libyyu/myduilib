...
inline void  write_log_impl(CLog::Log_Level lev, const char* fmt, va_list& va)
{
	if (!theLog) return;
	(*theLog)(lev, fmt, va);
}
...
#ifdef _UNICODE
	#define  write_log(lev,fmt,...)  (write_log_impl(lev,fmt,##__VA_ARGS__))
#else
	#define  write_log(lev,fmt,...)  (write_log_impl(lev,_T("[%s][%d]")fmt,__FILET__,__LINE__,##__VA_ARGS__))
#endif
