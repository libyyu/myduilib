...
namespace DuiLib
{
	...
	FGetClassObject UILIB_API RegisterComDll(LPCTSTR strPathName)
	{
		...
		if (!hMod) {
			hMod = ::LoadLibraryEx(strPathName, NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
			if(!hMod)
				return NULL;
		}
		typedef HRESULT(CALLBACK *FREG2)();
		HRESULT hResult;
		FREG2 lpfunc = (FREG2)::GetProcAddress(hMod, _T("DllRegisterServer"));
		if (!lpfunc)
		{
			::FreeLibrary(hMod);
			return NULL;
		...
		FGetClassObject pFunc = (FGetClassObject)::GetProcAddress(hMod, _T("DllGetClassObject"));
		return pFunc;
	}
		...
		COLORREF clrBg, BOOL bAutoScale, int nReservedWidth)
	{
		...
		{
			if (g_pcf)
				hr = g_pcf->CreateInstance(NULL, IID_IImageOle, (void **)&pImageOle);
			if (FAILED(hr))
				goto Ret0;
		}
