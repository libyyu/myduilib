...
public:
    ...
    void GetItemText(CControlUI* pControl, int iIndex, int iSubItem, CDuiString& text)
    {
        if( pControl->GetParent()->GetParent()->GetName() == _T("userlist") ) {
            if( iSubItem == 0 ) text = _T("<i vip.png>");
            if( iSubItem == 1 ) text = _T("<i vip.png>");
            if( iSubItem == 2 ) text = _T("此人昵称");
            if( iSubItem == 3 ) text = _T("5");
            if( iSubItem == 4 ) text = _T("50%");
            if( iSubItem == 5 ) text = _T("0%");
            if( iSubItem == 6 ) text = _T("100");
        }
...
extern "C"
{
    int __declspec(dllexport) RunGameDemo(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
    {
        return RunApp(hInstance, hPrevInstance, lpCmdLine, nCmdShow);
    }
}
