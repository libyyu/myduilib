...
//	if(m_pSocketListen->Create(mci::ToUInt(sPort),1,sIP))
	...
	if(m_pSocketListen->Create(mci::ToUInt(sPort),SOCK_STREAM))
	{
		...
		FormatMsgToEditView(_T("(系统消息)"),mci::format_currenttime(),CDuiString(_T("成功监听:")) + sIP + _T(":") + sPort);
	}
	else
	{
...
//			m_listMsg.AddString(anormalMsg);//在消息框中显示消息
			sLog(_T("Receive:用户[%s]发送群消息..."), m_pMsg->m_nomalMsg.who);
		}
		...
		else if(preType==MSGTYPE::SYSTEMMSG)
		{
			...
			{//注册
				...
				SendMsg(sf,pSocket);
				sLog(_T("Receive:用户[%s]注册消息..."), m_pMsg->m_systemMsg.name);
				FormatMsgToEditView(_T("(系统消息)"),m_pMsg->m_systemMsg.sndTime,CDuiString(_T("用户:"))+m_pMsg->m_systemMsg.name+_T(" 注册..."));
			}
			else if (m_pMsg->m_systemMsg.type==_T("SHAKE"))
			{//震动
				CDuiString sf = BuildShakeMsg(m_pMsg->m_systemMsg.name);
				SendMsgToAll(sf);
				sLog(_T("Receive:用户[%s]发送抖动消息..."), m_pMsg->m_systemMsg.name);
				FormatMsgToEditView(_T("(系统消息)"),m_pMsg->m_systemMsg.sndTime,CDuiString(_T("用户:"))+m_pMsg->m_systemMsg.name+_T(" 发送抖动消息..."));
			}
			...
			{//登录
				...
				if(!IsUserLoged(m_pMsg->m_systemMsg.name,sl))
				{
					...
					if(ret)
					{
						...
						SendMsgToAll(a);
						Sleep(500);
						CDuiString userList = BuildUserListMsg();//生成用户列表
						SendMsgToAll(userList);//给所有用户发送列表

						sLog(_T("Receive:用户[%s]登录成功消息..."), m_pMsg->m_systemMsg.name);
						FormatMsgToEditView(_T("(系统消息)"),m_pMsg->m_systemMsg.sndTime,CDuiString(_T("用户:"))+m_pMsg->m_systemMsg.name+_T(" 登录虾聊..."));
					}
					else
					{
						SendMsg(sf,pSocket);
	//					DeleteSocket(pSocket);
						sLog(_T("Receive:用户[%s]登录失败消息..."), m_pMsg->m_systemMsg.name);
					}
				}
				else
				{
					SendMsg(sl,pSocket);
					sLog(_T("Receive:用户[%s]重登录消息..."), m_pMsg->m_systemMsg.name);
				}
			}
			else if(m_pMsg->m_systemMsg.type==MSGTYPE::CLOSE)
			{//客户端退出
//				CDuiString a = _T("系统消息(CL)") + m_pMsg->m_systemMsg.name + _T("  退出了聊天室!");
				sLog(_T("Receive:用户[%s]退出消息..."), m_pMsg->m_systemMsg.name);
				FormatMsgToEditView(_T("(系统消息)"),m_pMsg->m_systemMsg.sndTime,CDuiString(_T("用户:"))+m_pMsg->m_systemMsg.name+_T(" 退出虾聊..."));
...
// 				CDuiString a = _T("系统消息(CH)") + m_pMsg->m_systemMsg.name + _T("  改名为  ") + m_pMsg->m_systemMsg.msgItem;
// 				a = AddTimeMsg(a);
// 				SendMsgToAll(a);
				//立即更新界面信息
				...
				CDuiString userList = BuildUserListMsg();
				SendMsgToAll(userList);//立即更新客户端列表信息
				sLog(_T("Receive:用户[%s]更名消息..."), m_pMsg->m_systemMsg.name);
				FormatMsgToEditView(_T("(系统消息)"),m_pMsg->m_systemMsg.sndTime,CDuiString(_T("用户:"))+m_pMsg->m_systemMsg.name+_T(" 更名为：")+m_pMsg->m_systemMsg.msgItem);
