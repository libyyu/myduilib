...
//		_itoa(nValue,buf,10);
	...
	int CDatabase::ExecScalar(const char* fmt,...)
	{
		...
		try
		{
			...
			va_end(va);
			nResult = _pDB->execScalar((const char*)buffer);
		}
		catch (CppSQLite3Exception e)
		{
			assert(false);
	...
	int CDatabase::ExecSql(const char* fmt,...)
	{
		...
		try
		{
			...
			buffer.format(fmt,va);
			va_end(va);
			nResult = _pDB->execDML((const char*)buffer);
		}
		catch (CppSQLite3Exception e)
		{
			throw e;
	...
	CDatabase::SQLResult CDatabase::QuerySql(const char* fmt,...)
	{
		...
		try
		{
			...
			rs = _pDB->execQuery((const char*)buffer);
		}
		catch (CppSQLite3Exception e)
		{
			assert(false);
...
//			memcpy(UserInfo._passwd,rs.getStringField(1),strlen(rs.getStringField(1)));
			UserInfo.m_loadName = TO_DUISTRING(rs.getStringField(0));
			UserInfo.m_password = TO_DUISTRING(rs.getStringField(1));
	...
	bool CUserMgr::GetOtherUsersToList(std::vector<User> &UserList,User UserInfo)
	{
		...
		std::string sname = TO_STDSTRING(UserInfo.m_loadName);
	...
	bool CUserMgr::InsertUser(User UserInfo)
	{
		...
		std::string sname = TO_STDSTRING(UserInfo.m_loadName);
		std::string spasswd = TO_STDSTRING(UserInfo.m_password);
