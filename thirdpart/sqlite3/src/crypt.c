...
static LPCRYPTBLOCK CreateCryptBlock(HCRYPTKEY hKey, Pager *pager, LPCRYPTBLOCK pExisting)
{
  ...
  {
    pBlock = sqlite3Malloc(sizeof(CRYPTBLOCK));
  ...
  {
    pBlock->pvCrypt = sqlite3Malloc(pBlock->dwCryptSize + (CRYPT_OFFSET * 2));
  }
  return pBlock;
}
...
int sqlite3_rekey_interop(sqlite3 *db, const void *pKey, int nKeySize)
{
  ...
  if (!rc)
  {
   ...
   // int n_out_Page=-1;
	...
	DbPage* pPage = NULL;
    Pgno n;
    sqlite3PagerPagecount(p,&n_out_Page);
    nPage=(Pgno)n_out_Page;
