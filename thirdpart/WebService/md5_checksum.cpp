...
*****************************************************************************************/
#if defined(_MSC_VER)
std::string md5_checksum::get_md5(const TCHAR* p_file_path, unsigned int n_begin, unsigned int n_len)
{
	std::string s_hash;
	...
	if (n_result !=  INVALID_SET_FILE_POINTER)
	{
		...
		while (n_remains > 0)
		{
			n_needs = std::min(BUF_SIZE, n_remains); 
...
std::string md5_checksum::get_md5(const TCHAR* p_file_path)
{
	HANDLE h_file = ::CreateFile(p_file_path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (h_file == INVALID_HANDLE_VALUE)
		return "";
	...
	{
		n_need_bytes = std::min(BUF_SIZE, (unsigned int)n_total_bytes);
...
#else
std::string md5_checksum::get_md5(const char* p_file_path)
{
	int file = -1;
	if((file = open(p_file_path, O_RDONLY)) < 0)
		return "";
	long offset = lseek(file, 0L, SEEK_CUR);
	long filelen = lseek(file,0L,SEEK_END);    
    lseek(file, offset, SEEK_SET);
	long n_total_bytes = filelen - offset;
	if (n_total_bytes == 0)
	{
		close(file);
		return "";
	...
	while (n_total_bytes > 0)
	{
		n_need_bytes = std::min((long)BUF_SIZE, n_total_bytes);
		n_read_bytes = read(file, s_buf, n_need_bytes);
		assert(n_read_bytes != -1 && n_need_bytes == n_read_bytes);
		md5_check.Update(s_buf, n_read_bytes);
		n_total_bytes -= n_read_bytes;
	}
	close(file);
...
std::string md5_checksum::get_md5(const char* p_file_path, unsigned int n_begin, unsigned int n_len)
{
	int file = -1;
	if((file = open(p_file_path, O_RDONLY)) < 0)
		return "";
	...
	static const unsigned int BUF_SIZE = 1024;
	unsigned char s_buf[BUF_SIZE];
	int n_result = lseek(file, n_begin, SEEK_SET);
	if (n_result !=  -1)
	{
		unsigned int n_remains = n_len;
		unsigned int n_needs = 0;
		...
		{
			n_needs = std::min(BUF_SIZE, n_remains); 
			n_reads = read(file, s_buf, n_needs);
			assert(n_reads != -1 && n_needs == n_reads);
			md5_check.Update(s_buf, n_reads);
			n_remains -= n_reads;
		}
		s_hash = md5_check.Final();
...
std::string md5_checksum::finish()
{
	return Final();
}
...
*****************************************************************************************/
std::string md5_checksum::get_md5(const unsigned char* p_buf, unsigned int n_length)
{
	md5_checksum md5_check;
