...
bool Des_Go(char *Out, char *In, long datalen, const char *Key, int keylen,int &out_length, bool Type)
{
    if( !( Out && In && Key && (datalen=(datalen+7)&0xfffffff8) ) ) 
		return false;
        char Tmp_data[256];	
	bool SubKey[2][16][48];	//16位
	bool Is3DES=SetKey(Key, keylen,Tmp_data,&SubKey[0],&SubKey[1]);
   
	if( !Is3DES ) {   // 1次DES
		for(long i=0,j=datalen>>3; i<j; ++i,Out+=8,In+=8)
			DES(Out, In, &SubKey[0], Type,Tmp_data);
	} else{   // 3次DES 加密:加(key0)-解(key1)-加(key0) 解密::解(key0)-加(key1)-解(key0)
		for(long i=0,j=datalen>>3; i<j; ++i,Out+=8,In+=8) {
			DES(Out, In,  &SubKey[0], Type,Tmp_data);
			DES(Out, Out, &SubKey[1], !Type,Tmp_data);
			DES(Out, Out, &SubKey[0], Type,Tmp_data);
			out_length+=8;
