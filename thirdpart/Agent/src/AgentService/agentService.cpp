...
bool CAgentService::AsynSendPacket(unsigned int nIndex,const void* buf,size_t cnt)
{
	...
	spBufferT packet(new _FStd(FBuffer)(std::max<size_t>(cnt,4096)));
	assert(packet);
...
{
	assert(_s);
	spBufferT recvBuffer(new _FStd(FBuffer)(_nMaxPacketSize));
	while(1)
	{
		HSOCK theSock(_s);
		if(!theSock)
...
bool CAgentService::DoOnceSend(HSOCK hsession,CSockAddr& addr)
{
	...
	spBufferT recvBuffer(new _FStd(FBuffer));
