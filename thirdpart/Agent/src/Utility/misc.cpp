...
#else
	return std::string(s_result.GetString());
#endif
}
...
bool delete_file_undo(const wchar_t* p_file_name, bool b_file_only)
{
	bool b_result = false;
	int n_result = 0;
	wchar_t s_buf[MAX_PATH + 2];
	memset(s_buf, 0x0, sizeof(wchar_t) * (MAX_PATH + 2));
	if (p_file_name == NULL || wcslen(p_file_name) > MAX_PATH)
		return false;
	wcsncpy(s_buf, p_file_name, MAX_PATH);
	s_buf[wcslen(p_file_name)] = NULL;
	s_buf[wcslen(p_file_name) + 1] = NULL;
	...
	n_result = ::SHFileOperationW(&FileOp);
	if (n_result != 0)
	{
...
bool delete_dir(const wchar_t* p_dir_name)
{
	...
	bool b_result = false;
	unsigned int n_len = wcslen(p_dir_name);
	wchar_t* p_buf = new wchar_t[n_len + 2];
	memset(p_buf, 0x0, sizeof(wchar_t) * (n_len + 2));
	SHFILEOPSTRUCTW file_op;
	ZeroMemory((void*)&file_op, sizeof(SHFILEOPSTRUCTW));
	wcsncpy(p_buf, p_dir_name, n_len + 1);
	...
	b_result = SHFileOperationW(&file_op) == 0;
	delete[] p_buf;
	return b_result; 
}
...
CString rip_file_ext(const CString& s_path_file, bool b_lower)
{
	...
	if (!s_path_file.IsEmpty())
	{
		TCHAR s_buf[MAX_PATH];
		s_buf[0] = 0x0;
		_tsplitpath(s_path_file.GetString(), NULL, NULL, NULL, s_buf);
		if (_tcslen(s_buf) > 0)
		{
			s_ext = s_buf;
			if (b_lower)
...
#else
	s_result = std::string(s_uni.GetString());
#endif
	return s_result;
}
...
ver_info get_version_info(const wchar_t* p_module_name)
{
	ver_info info;
	info.n_major_1 = info.n_major_2 = info.n_minor_1 = info.n_minor_2 = 0;
	if (p_module_name == NULL || wcslen(p_module_name) == 0)
		return info;
	unsigned int n_info_size = sizeof(VS_FIXEDFILEINFO);
	int n_ver_size = ::GetFileVersionInfoSizeW(p_module_name, NULL);
	if (n_ver_size != 0)
	{
		char* p_buf = new char[n_ver_size];
		if (::GetFileVersionInfoW(p_module_name, 0, n_ver_size, p_buf))
		{
			VS_FIXEDFILEINFO* p_vs_info = NULL;
