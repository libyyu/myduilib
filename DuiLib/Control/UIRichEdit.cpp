...
public:
	...
	static CTextServiceHelper* Instance()
	{
		static CTextServiceHelper inst;
		return &inst;
	}
protected:
	CTextServiceHelper()
	{
		hModuleRiched20 = ::LoadLibrary(_T("Riched20.dll"));	// ¼ÓÔØRichEdit¿Ø¼þDLL
		if (hModuleRiched20)
...
IOleObject * CImageDataObjectHandler::GetOleObject(IOleClientSite *pOleClientSite, IStorage *pStorage)
{
	...
	sc = ::OleCreateStaticFromData((LPDATAOBJECT)this, IID_IOleObject, OLERENDER_FORMAT,
...
#else
	hr = CTextServiceHelper::Instance()->CreateTextServices(NULL, this, &pUnk);
#endif
...
CRichEditUI::CRichEditUI() : CContainerUI(), m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true),
    m_bWantCtrlReturn(true), m_bTransparent(true), m_bRich(true), m_bReadOnly(false), m_bWordWrap(false), m_dwTextColor(0), m_iFont(-1), 
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bDrawCaret(true), m_bInited(false), m_pImageObjectCB(NULL)
{
