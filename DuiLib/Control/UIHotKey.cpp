...
namespace DuiLib
{
	...
	//////////////////////////////////////////////////////////////////////////
	CHotKeyUI::CHotKeyUI() : CLabelUI(), m_pWindow(NULL), m_wVirtualKeyCode(0), m_wModifiers(0), m_uButtonState(0), m_dwHotKeybkColor(0xFFFFFFFF)
	{
	...
	{
		return m_diNormalImage.sDrawString;
	}


	void CHotKeyUI::SetNormalImage(LPCTSTR pStrImage)
	{
		if (m_diNormalImage.sDrawString == pStrImage && m_diNormalImage.pImageInfo != NULL) return;
		m_diNormalImage.Clear();
		m_diNormalImage.sDrawString = pStrImage;
		Invalidate();
	}
	...
	{
		return m_diHotImage.sDrawString;
	}


	void CHotKeyUI::SetHotImage(LPCTSTR pStrImage)
	{
		if (m_diHotImage.sDrawString == pStrImage && m_diHotImage.pImageInfo != NULL) return;
		m_diHotImage.Clear();
		m_diHotImage.sDrawString = pStrImage;
		Invalidate();
	}
	...
	{
		return m_diFocusedImage.sDrawString;
	}


	void CHotKeyUI::SetFocusedImage(LPCTSTR pStrImage)
	{
		if (m_diFocusedImage.sDrawString == pStrImage && m_diFocusedImage.pImageInfo != NULL) return;
		m_diFocusedImage.Clear();
		m_diFocusedImage.sDrawString = pStrImage;
		Invalidate();
	}
	...
	{
		return m_diDisabledImage.sDrawString;
	}


	void CHotKeyUI::SetDisabledImage(LPCTSTR pStrImage)
	{
		if (m_diDisabledImage.sDrawString == pStrImage && m_diDisabledImage.pImageInfo != NULL) return;
		m_diDisabledImage.Clear();
		m_diDisabledImage.sDrawString = pStrImage;
		Invalidate();
	}
	...
	{
		if (m_cxyFixed.cy == 0) return SIZE{ m_cxyFixed.cx, m_pManager->GetFontInfo(GetFont())->tm.tmHeight + 6 };
	...
	void CHotKeyUI::PaintStatusImage(HDC hDC)
	{
		...
		if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
			if( !m_diDisabledImage.sDrawString.IsEmpty() ) {
				if( !DrawImage(hDC, m_diDisabledImage) ) m_diDisabledImage.Clear();
				else return;
			}
		}
		else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
			if( !m_diFocusedImage.sDrawString.IsEmpty() ) {
				if( !DrawImage(hDC, m_diFocusedImage) ) m_diFocusedImage.Clear();
				else return;
			}
		}
		else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
			if( !m_diHotImage.sDrawString.IsEmpty()) {
				if( !DrawImage(hDC, m_diHotImage) ) m_diHotImage.Clear();
				else return;
			}
		}


		if( !m_diNormalImage.sDrawString.IsEmpty() ) {
			if( !DrawImage(hDC, m_diNormalImage) ) m_diNormalImage.Clear();
			else return;
		}
	}
