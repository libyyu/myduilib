...
namespace DuiLib
{
	...
	void CFlashUI::DoInit()
	{
		...
		if (m_pFlash && !m_szFlashUrl.IsEmpty())
		{
			CDuiString szURL = Path::CombinePath(CPaintManagerUI::GetInstancePath(), m_szFlashUrl);
			m_pFlash->put_WMode(_bstr_t(_T("Transparent")));
			m_pFlash->put_Movie(_bstr_t(szURL));
