...
namespace DuiLib {
	...
	static std::map<CDuiString, ControlCreator>& GetDuiClassMaps()
	{
	...
	void CControlFactory::RegisterDuiClass(LPCTSTR pstrClassName, ControlCreator _createDuiInstanceFun, LPCTSTR category)
	{
		...
		{
			DuiLogWarning(_T("dumplicate register control class: %s."), pstrClassName);
			return;
		}
	...
	void* CControlFactory::createInstance(LPCTSTR pstrClassName)
	{
		if (!isRegister(pstrClassName))
		{
			//DuiLogError(_T("CControlFactory: control class %s not found."), pstrClassName);
			return NULL;
		}
		else
		{
...
m_nTooltipWidth(300),
m_pTagCtrl(NULL),
m_pBindEventCtrl(NULL),
m_pBindNotifyCtrl(NULL)
{
    m_cXY.cx = m_cXY.cy = 0;
...
CDuiString CControlUI::ToString() const
{
	TCHAR szBuf[MAX_PATH] = { 0 };
	_stprintf_s(szBuf, MAX_PATH - 1, _T("%s:%s"), GetClass(), m_sName.GetData());
	return szBuf;
}
...
bool CControlUI::IsName(LPCTSTR pstrName)
{
	return _tcscmp(pstrName, m_sName) == 0;
}
...
bool CControlUI::IsClass(LPCTSTR pstrName)
{
	return _tcscmp(pstrName, GetClass()) == 0;
}
...
void CControlUI::AddCustomAttribute(LPCTSTR pstrName, LPCTSTR pstrAttr)
{
	...
	if (pCostomAttr != NULL) {
		CDuiString* pOldAttr = static_cast<CDuiString*>(m_mCustomAttrHash.Set(pstrName, (LPVOID)pCostomAttr));
...
{
	if (m_pBindNotifyCtrl) m_pBindNotifyCtrl->DoNotify(msg);
	else if (OnNotify(&msg)) DoNotify(msg);
}
void CControlUI::DoNotify(TNotifyUI& msg)
{
...
{
	if (m_pBindEventCtrl) m_pBindEventCtrl->DoEvent(event);
    else if( OnEvent(&event) ) DoEvent(event);
}
...
{
	if (!pstrStyleName || _tclen(pstrStyleName) <= 0 || !m_pManager)
		return;

	if (m_sStyleName == pstrStyleName) return;

	CDuiStringPtrMap* pStyleAttributeHash = m_pManager->GetStyleAttributeList(pstrStyleName);
	if (!pStyleAttributeHash)
		return;

	for (int i = 0; i<pStyleAttributeHash->GetSize(); ++i)
	{
		CDuiString* pStrAttrValue = NULL;
		LPCTSTR pStrAttrKey = NULL;
...
void CControlUI::SetBindEventCtrl(CControlUI* pControl)
{
	m_pBindEventCtrl = pControl;
}
void CControlUI::SetBindNotifyCtrl(CControlUI* pControl)
{
