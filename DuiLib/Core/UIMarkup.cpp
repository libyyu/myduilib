...
bool CMarkup::Load(LPCTSTR pstrXML)
{
    Release();
	CResource ResLoader;
	BOOL bRes = ResLoader.LoadFromData(pstrXML);
	if (!bRes)
	{
		Release();
		return false;
	}
	return _BeginParse(&ResLoader);
}

bool CMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
	CResource ResLoader;
	if (!ResLoader.LoadFromMem(pByte, dwSize))
		return false;
...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
	...
	CResource ResLoader;
	if (!ResLoader.LoadFromFile(pstrFilename))
		return false;

	return _BeginParse(&ResLoader, encoding);
}

bool CMarkup::LoadFromFile(LPCTSTR pstrPath, LPCTSTR pstrFilename, int encoding)
{
	Release();

	CResource ResLoader;
	if (!ResLoader.LoadFromFileDirectly(pstrPath, pstrFilename))
		return false;
...
bool CMarkup::_BeginParse(CResource* ResLoader, int encoding)
{
	LPBYTE pByte = ResLoader->GetData();
...
{
    // Register last error
    DuiLogError(_T("XML Error: %s"), pstrError);
    if( pstrLocation != NULL ) DuiLogError(pstrLocation);
