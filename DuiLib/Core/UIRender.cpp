...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
	...
	do 
	{
		if (type != NULL && _tcscmp(type, RES_TYPE_COLOR) == 0) {
			pData = (PBYTE)0x1;  /* dummy pointer */
		}
		else {
			if (!CResourceManager::Instance()->LoadAsset(bitmap.m_lpstr, type, &pData, &dwSize))
			{
				DuiLogError(_T("(LoadImage)Failed LoadImage %s"), bitmap.m_lpstr);
				return NULL;
			}
		}
	} while (0);
    ...
    if (!type || _tcscmp(type, RES_TYPE_COLOR) != 0) {
        ...
        if( !pImage ) {
			...
			DuiLogError(_T("(LoadImage)(stbi_load_from_memory)Failed LoadImage %s."), bitmap.m_lpstr);
            return NULL;
        }
    }
    BITMAPINFO bmi;
	...
	if( !hBitmap ) {
		...
		DuiLogError(_T("(LoadImage)(CreateDIBSection)Failed LoadImage %s"), bitmap.m_lpstr);
		return NULL;
	}
