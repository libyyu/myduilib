...
#else
		pNode->ToElement()->SetAttribute("myname", pControl->GetName());
#endif
...
#else
		pNode->ToElement()->SetAttribute("parentname", pParent->GetName());
#endif
	}
}
...
#else
	TiXmlElement* pNode = new TiXmlElement(strClass.GetBuffer());
	pNode->SetAttribute("myname", m_pControl->GetName());
...
#else
	CControlUI* pParentControl = pManager->FindControl(strParentName);
#endif
...
#else
	CControlUI* pRoot = builder.Create(printer.CStr(), (UINT)0, NULL, pManager);
#endif
 	if(pRoot)
...
void CALLBACK CUICommandHistory::UIModify(TiXmlNode* pNode)
{
...
#else
	CControlUI* pControl = pManager->FindControl(strName);
#endif
	...
	while(pAttrib)
	{
		if(strcmp(pAttrib->Name(), "name") == 0)
		{
			...
			g_pClassView->RenameUITreeItem(pControl, UTF8_TO_UNICODE_STR(pAttrib->Value()));
			pControl->SetXMLAttribute(UTF8_TO_UNICODE_STR(pAttrib->Name())
				, UTF8_TO_UNICODE_STR(pAttrib->Value()));
...
void CALLBACK CUICommandHistory::UIDelete(TiXmlNode* pNode)
{
	...
	CUIDesignerView* pUIView = g_pMainFrame->GetActiveUIView();
	CPaintManagerUI* pManager = pUIView->GetPaintManager();
	CControlUI* pControl = pManager->FindControl(UTF8_TO_UNICODE_STR(strName));
	if(pControl == NULL)
		return;
	CControlUI* pParent = pControl->GetParent();
