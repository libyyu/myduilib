...
void CDialogCustomFonts::OnBnClickedButtonFontAdd()
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		DWORD count = m_pManager->GetCustomFontCount(false);
		m_pManager->AddFont(count+1, lf.lfFaceName, -lf.lfHeight, (lf.lfWeight == FW_BOLD), lf.lfUnderline!=0, lf.lfItalic!=0);
...
void CDialogCustomFonts::OnBnClickedButtonFontDelete()
{
	...
	if(MessageBox(_T("删除此字体后，使用了其后字体的控件，\n均需手动修改序号，请慎用此功能！")
		...
		return;
	bool bFind = !!m_pManager->GetFont(nIndex);
