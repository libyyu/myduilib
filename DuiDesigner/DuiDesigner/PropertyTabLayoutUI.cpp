...
{
	ASSERT(pControl);
	m_pTablayoutUI = static_cast<CTabLayoutUI*>(pControl->GetInterface(DUI_CTR_TABLAYOUT));
	ASSERT(m_pTablayoutUI);
}
...
{//FIXME
	...
	*/
	...
	CControlUI* pNewControl = g_pMainFrame->GetActiveUIView()->NewUI(strClass,rect,m_pTablayoutUI);
