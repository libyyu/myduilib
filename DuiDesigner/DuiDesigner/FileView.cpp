...
void CFileView::LoadUITree(CString& strPath)
{
	...
	xmlDoc.LoadFile(CSTRING_TO_ANI(strPath));
...
void CFileView::LoadUITree(TiXmlElement* pElement, HTREEITEM hParent)
{
	...
	while(pNode)
	{
		if(pNode->Type() == TiXmlNode::ELEMENT)
		{
			...
			{
				hItem = m_wndFileView.InsertItem(UTF8_TO_UNICODE_STR(strName), 2, 2, hParent);
				dwInfo = INFO_FILE;
			}
			else if(strcmp(pNode->Value(), "Directory") == 0)
			{
				hItem = m_wndFileView.InsertItem(UTF8_TO_UNICODE_STR(strName), 1, 1, hParent);
...
void CFileView::SaveProject()
{
	...
	TiXmlDocument xmlDoc(CSTRING_TO_ANI(strPathName));
	TiXmlDeclaration Declaration("1.0","utf-8","yes");
	xmlDoc.InsertEndChild(Declaration);

	TiXmlElement xmlElem("DirectUIProject");
	xmlElem.SetAttribute("Name", CSTRING_TO_UTF8(CGlobalVariable::m_strProjectName));
	xmlElem.SetAttribute("Version", CSTRING_TO_UTF8(UIDESIGNER_VERSION));
...
void CFileView::SaveUITree(HTREEITEM hItem, TiXmlElement* pParentNode)
{
	...
	while(hChild != NULL)
	{
		...
		if(dwInfo == INFO_FILE)
		{
			 ...
			 pNode->SetAttribute("Name", CSTRING_TO_UTF8(strName));
