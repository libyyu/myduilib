...
void CUIDesignerView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		CUITracker tracker;
		CString strClass = g_pToolBoxWnd->GetCurSel()->GetClass();
		CRect rect;
		...
		if(!strClass.IsEmpty())
		{
			CControlUI* pNewControl = m_LayoutManager.NewUI(strClass, rect, pControl, &m_LayoutManager);
...
CTrackerElement* CUIDesignerView::CreateTracker(CControlUI* pControl)
{
	...
	int nType = GetControlType(pControl);
	if(nType == UITypeForm)
		pTracker = new CTrackerElement(pControl, UITypeForm,this);
	else if(nType == UITypeContainer)
		pTracker = new CTrackerElement(pControl, UITypeContainer,this);
	else
		pTracker = new CTrackerElement(pControl,UITypeControl,this);
...
int CUIDesignerView::GetControlType(CControlUI* pControl)
{
	if(pControl->GetInterface(_T("Form")))
		...
		return UITypeContainer;

	return UITypeControl;
}
...
CControlUI* CUIDesignerView::NewUI(CString strClass,CRect& rect,CControlUI* pParent)
{
	CControlUI* pControl = m_LayoutManager.NewUI(strClass, rect, pParent, &m_LayoutManager);
	if(pControl != NULL)
		this->GetDocument()->SetModifiedFlag();
...
void CUIDesignerView::InitUI(CControlUI* pControl, int depth)
{
	...
	pExtended->strClass = pControl->GetClass();
