...
BOOL CDialogDefaultAttribList::OnInitDialog()
{
	...
	ASSERT(m_pManager);
	//TODO:
	DWORD nCount = m_pManager->GetDefaultAttributeListCount(false);
	for (int i=0; i<nCount; ++i)
	{
		LPCTSTR pstrName = m_pManager->GetDefaultAttributeListName(i, false);
...
void CDialogDefaultAttribList::OnBnClickedButtonAttribModify()
{
	...
	if(pControl == NULL)
	{
		if(strDefaultAttribName.IsEmpty())
		{
	...
	if (!pCopyControl)
	{
		MessageBox(_T("控件克隆失败！"), _T("提示"), MB_ICONINFORMATION);
		return;
	}
