...
CToolElement::CToolElement(const CString& strName, CString strClass,UINT nIDIcon):m_strName(strName),
m_strClass(strClass)
{
	m_bTab = FALSE;
	m_bExpanded = FALSE;
...
CToolElement* CToolElement::GetTool(CString strClass) const
{
	ASSERT_VALID(this);
...
void CToolBoxCtrl::SetCurSel(CString strClass,BOOL bRedraw/*=TRUE*/)
{
	ASSERT_VALID(this);
	...
	for (POSITION pos = m_lstToolTabs.GetHeadPosition(); pos != NULL;)
	{
		...
		pTool = pToolTab->GetTool(strClass);
		if(pTool)
		{
