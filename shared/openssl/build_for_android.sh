...
source_dir=openssl-1.0.2l

if [ -d $source_dir ]; then
  rm -rf $source_dir
fi

tar xzf $current_path/openssl-1.0.2l.tar.gz
if [ $? != 0 ]; then
   exit 1
fi

# 自己的android-toolchain(NDK针对特定配置抽取出来的独立目录)
export ANDROID_NDK_ROOT=$ndk_toolchain_dir
export PATH=$PATH:$ndk_toolchain_dir/bin

export ssl_path="$current_path/out/$target_cpu"
#####################################################################
...
# Error checking
if [ ! -d "$ANDROID_NDK_ROOT/bin" ]; then
...
do
  if [ -d "$ANDROID_NDK_ROOT/bin" ]; then
    ANDROID_TOOLCHAIN="$ANDROID_NDK_ROOT/bin"
    break
  fi
done
...
# https://android.googlesource.com/platform/ndk/+/ics-mr0/docs/STANDALONE-TOOLCHAIN.html
export ANDROID_SYSROOT="$ANDROID_NDK_ROOT/sysroot/usr"
export ANDROID_SYSTEM="$ANDROID_SYSROOT/include"
...
export SYSTEM="$ANDROID_SYSTEM"

# CROSS_COMPILE and ANDROID_DEV are DFW (Don't Fiddle With). Its used by OpenSSL build system.
# export CROSS_COMPILE="arm-linux-androideabi-"
export ANDROID_DEV="$ANDROID_SYSROOT"
export HOSTCC=gcc
...
# ./config no-asm no-shared no-cast no-idea no-camellia no-comp no-hw no-engine --openssldir=$ssl_path --prefix=$ssl_path 
# if [ $? != 0 ]; then
#     exit 1
# fi

#SYSTEM=android ARCH=arch-x86 ./Configure android-x86 no-shared no-comp no-hw no-engine --prefix=$ssl_path --openssldir=$ssl_path --sysroot=$CROSS_SYSROOT -D__ANDROID_API__=18 -isystem$ANDROID_SYSTEM
