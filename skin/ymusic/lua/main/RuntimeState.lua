...
--	self._spWinNotifyCenter = nil
	...
	self._spPlayCenter = nil
	self._spPlayListMgr = nil
...
function CRuntimeState:Init(dwMainThreadId)
	...
	--播放器类
	local AVPlayerT = require "playmgr.AVPlayer"
	self._spPlayCenter = AVPlayerT()
	assert(self._spPlayCenter:Init())
	--播放列表管理器
	local PlayListMgrT = require "playmgr.PlayListMgr"
	self._spPlayListMgr = PlayListMgrT()
...
function CRuntimeState:GetPlayCenter()
	return self._spPlayCenter
end
function CRuntimeState:GetPlayListMgr()
	return self._spPlayListMgr
end
function CRuntimeState:GetLrcMgr()
end
